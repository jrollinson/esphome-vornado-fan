...
class VornadoController : public Component {
 public:
  void setup() override {
    ...
    // Validate required buttons are configured
    if (!power_button_) ESP_LOGW(TAG, "Power button not configured!");
    if (!direction_button_) ESP_LOGW(TAG, "Direction button not configured!");
    if (!increase_button_) ESP_LOGW(TAG, "Increase button not configured!");
    if (!decrease_button_) ESP_LOGW(TAG, "Decrease button not configured!");
  }
  ...
  void set_power_button(button::Button *button) { this->power_button_ = button; }
  void set_direction_button(button::Button *button) { this->direction_button_ = button; }
  void set_increase_button(button::Button *button) { this->increase_button_ = button; }
  void set_decrease_button(button::Button *button) { this->decrease_button_ = button; }
 ...
 protected:
  // Button references
  button::Button *power_button_{nullptr};
  button::Button *direction_button_{nullptr};
  button::Button *increase_button_{nullptr};
  button::Button *decrease_button_{nullptr};
  ...
  // Press button and update last command time
  ...
  // Press button, update time, and schedule lock release (common pattern)
  ...
  uint32_t calculate_ensure_on_wait() {
    ...
    if (last_command_time_ == 0) {
      // First command since boot - use boot time as reference
      // We need ensure_delay_ms_ to have passed since boot (last possible user interaction)
  ...
  // Helper: Press the appropriate button
  void press_button(int button_id) {
    switch (button_id) {
      case VornadoButton::POWER_ON:
      case VornadoButton::POWER_OFF:
        if (this->power_button_ != nullptr) {
          ESP_LOGD(TAG, "Pressing power toggle button");
          this->power_button_->press();
        }
        break;
      case VornadoButton::DIRECTION:
        if (this->direction_button_ != nullptr) {
          ESP_LOGD(TAG, "Pressing direction button");
          this->direction_button_->press();
        }
        break;
      case VornadoButton::SPEED_INCREASE:
        if (this->increase_button_ != nullptr) {
          ESP_LOGD(TAG, "Pressing speed increase button");
          this->increase_button_->press();
        }
        break;
      case VornadoButton::SPEED_DECREASE:
        if (this->decrease_button_ != nullptr) {
          ESP_LOGD(TAG, "Pressing speed decrease button");
          this->decrease_button_->press();
        }
        break;
      default:
        ESP_LOGW(TAG, "Invalid button_id: %d", button_id);
        break;
    }
  }
  ...
  void handle_ensure_on() {
    ...
    // Lambda to execute the power on (avoids duplication)
